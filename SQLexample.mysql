...
-- Create stored procedure to fetch employee name based on emp_id
DELIMITER //

CREATE PROCEDURE GetEmployeeName(IN emp_id INT, OUT emp_name VARCHAR(128))
BEGIN
...
-- Create function to fetch department name based on dept_id
DELIMITER //

CREATE FUNCTION GetDepartmentName(dept_id INT) RETURNS VARCHAR(128)
BEGIN
    DECLARE dept_name VARCHAR(128);
...
-- Example calls to the procedure and function
CALL GetEmployeeName(1, @emp_name);
SELECT @emp_name;

SELECT GetDepartmentName(1) AS DepartmentName;
